        ...
        try {
            ...
            if ($resultset != false) {
                foreach ($resultset as $r) {
                    $locationlist[] = Location::create(intval($r["id"]), $r["name"], $r["description"], $r["start"], $r["end"], $r["level"]);
                }
                return $locationlist;
            } else {
                return false;
...
// getById tested ovdb
    public static function getById($id) {
        try {
            ...
            $r = $pstmt->fetch();
            if ($r != false) {
                return Location::create(intval($r["id"]), $r["name"], $r["description"], $r["start"], $r["end"], $r["level"]);
            } else {
                return false;
            }
        } catch (\PDOException $e) {
